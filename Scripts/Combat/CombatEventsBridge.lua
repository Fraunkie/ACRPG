...
-- • Souls: on KILL only
-- • SpiritDrive: on HIT and on KILL bonus (PLAYER-ONLY)
--==================================================
...
do
    ...
    local function pidOfUnit(u)
        ...
        if pid < 0 or pid >= bj_MAX_PLAYERS then return nil end
        return pid
    end
    ...
    local function AddThreat(source, target, amount)
        ...
        if type(TS.AddThreat) == "function" then
            ...
            -- prefer pid-inclusive signature if present
            local pid = pidOfUnit(source) or -1
            local ok = pcall(TS.AddThreat, pid, target, val, "combat")
            if not ok then pcall(TS.AddThreat, source, target, val) end
    ...
    --------------------------------------------------
    -- XP helper (SoulEnergyLogic / HFILUnitConfig / fallback)
    --------------------------------------------------
    local function RewardXP(killer, dead)
        ...
        elseif rawget(_G, "HFILUnitConfig") and HFILUnitConfig.GetByTypeId then
            local row = HFILUnitConfig.GetByTypeId(raw)
        ...
        local kpid = pidOfUnit(killer); if not IsHumanPid(kpid) then return end
    ...
    local function hookDamageEngine()
        ...
        if evBefore then
            DamageEngine.registerEvent(evBefore, function()
                ...
                if onHitSD ~= 0 and IsHumanPid(pid) and _G.SpiritDrive and SpiritDrive.Add then
                    pcall(SpiritDrive.Add, pid, onHitSD)
                end
            end)
        end

        local function lethalHandler(d)
            if not d then return end
            ...
            if IsHumanPid(kpid) then markCombat(kpid) end
            ...
            if bonus ~= 0 and IsHumanPid(kpid) and _G.SpiritDrive and SpiritDrive.Add then
        ...
        if evLethal then
            DamageEngine.registerEvent(evLethal, function()
                lethalHandler(DamageEngine.getCurrentDamage and DamageEngine.getCurrentDamage() or nil)
            end)
        elseif evAfter then
            DamageEngine.registerEvent(evAfter, function()
                local d = DamageEngine.getCurrentDamage and DamageEngine.getCurrentDamage() or nil
                if not d then return end
                if ValidUnit(d.target) and GetWidgetLife(d.target) > 0.405 then return end
                lethalHandler(d)
            end)
        end
